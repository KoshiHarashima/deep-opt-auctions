...
def benchmark_device(device_name, device, cfg, Net, Generator, clip_op_lambda, Trainer, num_iterations=1000, warmup_iterations=10):
    ...
    # Create trainer
    trainer = Trainer(cfg, "train", net, clip_op_lambda)
    ...
    if hasattr(trainer, 'adv_var'):
        trainer.adv_var.data = trainer.adv_var.data.to(device)
    if hasattr(trainer, 'adv_mask_tensor'):
        trainer.adv_mask_tensor = trainer.adv_mask_tensor.to(device)
    if hasattr(trainer, 'u_mask_tensor'):
        trainer.u_mask_tensor = trainer.u_mask_tensor.to(device)
    if hasattr(trainer, 'w_rgt'):
        trainer.w_rgt.data = trainer.w_rgt.data.to(device)
    if hasattr(trainer, 'update_rate'):
        trainer.update_rate.data = trainer.update_rate.data.to(device)
    
    # Warmup
    print(f"Warming up ({warmup_iterations} iterations)...")
    for _ in range(warmup_iterations):
        X, ADV, perm = next(generator[0].gen_func)
        X_tensor = torch.from_numpy(X).to(device).float()
        ADV_tensor = torch.from_numpy(ADV).to(device).float()
        trainer.adv_var.data.copy_(ADV_tensor)
...
def main():
    ...
    cpu_result = benchmark_device("CPU", cpu_device, cfg, Net, Generator, clip_op_lambda, Trainer, num_iterations)
    results.append(cpu_result)
    
    # Benchmark GPU if available
    if cuda_available:
        gpu_device = torch.device('cuda')
        gpu_result = benchmark_device("GPU", gpu_device, cfg, Net, Generator, clip_op_lambda, Trainer, num_iterations)
